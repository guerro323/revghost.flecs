...
public unsafe struct World : IDisposable
{
    ...
    {
        Handle = existing;
        ecs_set_entity_range(Handle, new ecs_entity_t {Data = StaticEntityInternal.MaxCount}, default);
    }
    ...
    private void Register(EntityId id, EntityId parent, string name)
    {
        ...
        {
            var data = StaticEntityInternal.GetData(parent);
        ...
        if (!didExists)
        {
            if (StaticEntityInternal.GetData(id).Setup is { } setup)
            {
    ...
    {
        var data = StaticEntityInternal.GetData(type);
