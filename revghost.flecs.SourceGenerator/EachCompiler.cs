...
""");
        ...
        public override SyntaxNode? VisitMemberAccessExpression(MemberAccessExpressionSyntax node)
        {
            if (GetFieldSymbolFrom(node) is { } tuple)
            {
                ...
                if (tuple.componentField.IsBuiltin)
                {
                    ...
                    {
                        return SyntaxFactory.ParseExpression(
                            $"Unsafe.As<__FLECS__.ecs_entity_t, EntityId>(ref {itVar}->entities[{indexName}]).WithWorld(global::revghost.flecs.World.FromExistingUnsafe({itVar}->world)){leftOver}"
                        );
                    }
        ...
        public override SyntaxNode VisitIdentifierName(IdentifierNameSyntax node)
        {
            ...
            if (index < 0)
            {
                ...
                if (name == "World")
                {
                    return SyntaxFactory.ParseExpression("global::revghost.flecs.World.FromExistingUnsafe(__it__->world)");
                }
                
                if (name == "RealWorld")
                {
                    return SyntaxFactory.ParseExpression("global::revghost.flecs.World.FromExistingUnsafe(__it__->real_world)");
                }
